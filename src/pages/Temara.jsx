...
export default function Temara() {
  const [temara, setTemara] = useState([]);
  const [search, setSearch] = useState("");
  const [weight, setWeight] = useState('');
  const [height, setHeight] = useState('');
  const [location, setLocation] = useState('Temara')
  ...
  async function fetchTemara() {
    const { data: temara } = await client.models.Temara.list();
    await Promise.all(
      temara.map(async (temarainfo) => {
        if (temarainfo.image) {
          const linkToStorageFile = await getUrl({
            path: ({ identityId }) => `media/${identityId}/${temarainfo.image}`,
          });
          console.log(linkToStorageFile.url);
          temarainfo.image = linkToStorageFile.url;
        }
        return temarainfo;
      })
    );
  ...
  async function createNote(event) {
    ...
    const { data: newTemara } = await client.models.Temara.create({
      patientId: primaryid,
    ...
    console.log(newTemara);
    if (newTemara.image)
      if (newTemara.image)
        await uploadData({
          path: ({ identityId }) => `media/${identityId}/${newTemara.image}`,
