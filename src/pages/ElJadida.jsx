...
export default function ElJadida() {
  const [elJadida, setElJadida] = useState([]);
  ...
  async function fetchElJadida() {
    const { data: elJadida } = await client.models.ElJadida.list();
    await Promise.all(
      elJadida.map(async (elJadidainfo) => {
        if (elJadidainfo.image) {
          const linkToStorageFile = await getUrl({
            path: ({ identityId }) => `media/${identityId}/${elJadidainfo.image}`,
          });
          console.log(linkToStorageFile.url);
          elJadidainfo.image = linkToStorageFile.url;
        }
        return elJadidainfo;
      })
    );
    console.log(elJadida);
    setElJadida(elJadida);
  }
  async function createNote(event) {
    ...
    const { data: newElJadida } = await client.models.ElJadida.create({
      patientId: primaryid,
    ...
    console.log(newElJadida);
    if (newElJadida.image)
      if (newElJadida.image)
        await uploadData({
          path: ({ identityId }) => `media/${identityId}/${newElJadida.image}`,
  ...
  async function deleteElJadida({ patientId }) {
    const tobeDeletedelJadida = {
      patientId: patientId,
    };

    const { data: deletedelJadida } = await client.models.ElJadida.delete(
        tobeDeletedelJadida
    );
    console.log(deletedelJadida);
