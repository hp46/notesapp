...
export default function Ouarzazate() {
  const [ouarzazate, setOuarzazate] = useState([]);
  const [search, setSearch] = useState("");
  const [weight, setWeight] = useState('');
  const [height, setHeight] = useState('');
  const [location, setLocation] = useState('Ouarzazate')
  ...
  async function fetchOuarzazate() {
    const { data: ouarzazate } = await client.models.Ouarzazate.list();
    await Promise.all(
      ouarzazate.map(async (ouarzazateinfo) => {
        if (ouarzazateinfo.image) {
          const linkToStorageFile = await getUrl({
            path: ({ identityId }) => `media/${identityId}/${ouarzazateinfo.image}`,
          });
          console.log(linkToStorageFile.url);
          ouarzazateinfo.image = linkToStorageFile.url;
        }
        return ouarzazateinfo;
      })
    );
    console.log(ouarzazate);
    setOuarzazate(ouarzazate);
  }
  async function createNote(event) {
    ...
    const { data: newOuarzazate } = await client.models.Ouarzazate.create({
      patientId: primaryid,
    ...
    console.log(newOuarzazate);
    if (newOuarzazate.image)
      if (newOuarzazate.image)
        await uploadData({
          path: ({ identityId }) => `media/${identityId}/${newOuarzazate.image}`,
