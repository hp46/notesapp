...
export default function Marrakech() {
  const [marrakech, setMarrakech] = useState([]);
  ...
  async function fetchMarrakech() {
    const { data: marrakech } = await client.models.Marrakech.list();
    await Promise.all(
        marrakech.map(async (marrakechinfo) => {
        if (marrakechinfo.image) {
          const linkToStorageFile = await getUrl({
            path: ({ identityId }) => `media/${identityId}/${marrakechinfo.image}`,
          });
          console.log(linkToStorageFile.url);
          marrakechinfo.image = linkToStorageFile.url;
        }
        return marrakechinfo;
      })
    );
    console.log(marrakech);
    setMarrakech(marrakech);
  }
  async function createNote(event) {
    ...
    const { data: newMarrakech } = await client.models.Marrakech.create({
    ...
    console.log(newMarrakech);
    if (newMarrakech.image)
      if (newMarrakech.image)
        await uploadData({
          path: ({ identityId }) => `media/${identityId}/${newMarrakech.image}`,
  ...
  async function deleteMarrakech({ id }) {
