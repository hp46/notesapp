...
export default function Khemisset() {
  ...
  async function fetchKhemisset() {
    const { data: khemisset } = await client.models.Khemisset.list();
    await Promise.all(
      khemisset.map(async (khemissetinfo) => {
        if (khemissetinfo.image) {
          const linkToStorageFile = await getUrl({
            path: ({ identityId }) => `media/${identityId}/${khemissetinfo.image}`,
          });
          console.log(linkToStorageFile.url);
          khemissetinfo.image = linkToStorageFile.url;
        }
        return khemissetinfo;
      })
    );
  ...
  async function createNote(event) {
    ...
    console.log(newKhemisset);
    if (newKhemisset.image)
      if (newKhemisset.image)
        await uploadData({
          path: ({ identityId }) => `media/${identityId}/${newKhemisset.image}`,
