...
export default function Sale() {
  ...
  const [location, setLocation] = useState('Sale')
  ...
  async function fetchSale() {
    const { data: sale } = await client.models.Sale.list();
    await Promise.all(
      sale.map(async (saleinfo) => {
        if (saleinfo.image) {
          const linkToStorageFile = await getUrl({
            path: ({ identityId }) => `media/${identityId}/${saleinfo.image}`,
          });
          console.log(linkToStorageFile.url);
          saleinfo.image = linkToStorageFile.url;
        }
        return saleinfo;
      })
    );
  ...
  async function createNote(event) {
    ...
    const { data: newSale } = await client.models.Sale.create({
      patientId: primaryid,
    ...
    if (newSale.image)
      if (newSale.image)
        await uploadData({
          path: ({ identityId }) => `media/${identityId}/${newSale.image}`,
