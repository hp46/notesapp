...
export default function Azour() {
  const [azour, setAzour] = useState([]);
  ...
  async function fetchAzour() {
    const { data: azour } = await client.models.Azour.list();
    await Promise.all(
      azour.map(async (azourinfo) => {
        if (azourinfo.image) {
          const linkToStorageFile = await getUrl({
            path: ({ identityId }) => `media/${identityId}/${azourinfo.image}`,
          });
          console.log(linkToStorageFile.url);
          azourinfo.image = linkToStorageFile.url;
        }
        return azourinfo;
      })
    );
  ...
  async function createNote(event) {
    ...
    const { data: newAzour } = await client.models.Azour.create({
      patientId: primaryid,
    ...
    console.log(newAzour);
    if (newAzour.image)
      if (newAzour.image)
        await uploadData({
          path: ({ identityId }) => `media/${identityId}/${newAzour.image}`,
              ...
              >
                ...
                <div className="flex flex-row w-[450px] justify-between">
                  <div>
                    ...
                    required
                    >
                    <option value="Azour">Azour</option>
                    </SelectField>
                  </div>
                  <div>
